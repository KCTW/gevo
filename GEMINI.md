...
4. **極致精簡 (US-10)**: 文字回覆嚴禁超過 3 行，嚴禁轉場與總結廢話。
5. **入庫完整性 (US-12)**: 修改配置或程式後，必須主動執行 git commit & push。
5. **入庫完整性 (US-12)**: 修改配置或程式後，必須主動執行 git commit & push。
6. **變更邊界 (US-13)**: 修改外部框架 (如 shikigami) 核心後，必須請求人工確認才可 push。
6. **變更邊界 (US-13)**: 修改外部框架 (如 shikigami) 核心後，必須請求人工確認才過 push。
7. **Context 隔離 (US-15)**: 執行測試時應優先參考本地文件，減少對話雜訊干擾。
7. **Context 隔離 (US-15)**: 執行測試時應優先參考本地文件，減少對話雜訊干擾。
